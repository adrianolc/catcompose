...
internal class DetailsRepository @Inject constructor(
    private val detailsService: DetailsService
) {
    suspend fun getCat(id: String): NetworkResult<Cat> = asNetworkResult {
        detailsService.getCat(id).toCat()
    }
}

private fun CatResponse.toCat(): Cat = Cat(
    id = id,
    url = url,
    breed = breeds.firstOrNull()?.toBreed()
)

private fun BreedResponse.toBreed(): Breed = Breed(
    id = id,
    name = name,
